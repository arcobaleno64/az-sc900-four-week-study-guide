...
記錄期間：2026-08-06。兩輪內容產出共送出 52 個背景 job。

## 一頁摘要

| #   | 問題                                                        | 嚴重度   | 是否有規避方式                 |
| --- | ----------------------------------------------------------- | -------- | ------------------------------ |
| 1   | `setup --json` 的就緒欄位互相矛盾，AGY 永遠無法回報 ready   | 低       | 有：直接送一個真實工作去試     |
| 2   | `task --help` 被當成提示詞送給模型，燒掉一次計費 turn       | 中       | 有：不要打 --help              |
| 3   | `gemini_job_result` 找不到 `gemini_job_status` 找得到的 job | 中       | 有：改用 CLI 的 result 子指令  |
| 4   | job 狀態卡在 queued 但 phase 已 done，result 永遠取不到     | 高       | 有：手動從 .log 挖             |
| 5   | 大型輸出逾時，卻標記為 retryable                            | 中       | 有：把批次拆小                 |
| 6   | 大型 diff 的 adversarial-review 只審一個檔就回報 approve    | **最高** | 有：改用 task 逐檔指名         |
| 7   | Windows 併發 job 撞 EPERM，最嚴重時輸出永久遺失             | **最高** | 部分：拉長間隔可減少但無法消除 |

**若只修一項**：#7 的第 2 點期望行為（`result` 改為掃描 `<jobId>.json` 而非依賴共用索引）。這一項同時消滅 #3、#4 的資料取不回，以及 #7 形態 B 以外的所有遺失。

**#6 與 #7 是性質不同的兩種嚴重**：#6 會讓使用者相信程式碼被審查過（沉默的假陰性），#7 會讓已經付費的輸出消失（可見的資料遺失）。
...
### #7 續：第二輪 27 個 job 的追加觀察（2026-08-06 下午）

同一天稍晚跑了第二輪內容產出（27 個背景 job，啟動間隔已拉到 9 秒）。EPERM 仍然出現，且暴露出兩個第一輪沒看到的形態。

**形態 A — 建立 job 時就失敗，job 從未存在**

```
EPERM: operation not permitted, rename
  '...\.state.json.39432.61b9f67a4c34.tmp' -> '...\state.json'
```

這次不是 worker 更新狀態時失敗，而是**在 `task --background` 建立 job 的當下**失敗。指令以非零狀態結束、沒有印出 job id，該工作等於沒送出。相較於形態 B 這反而是好的：使用者立刻看得到失敗。

注意目標檔名在這裡是 `state.json`（無前導點），而 #7 原始記錄是 `.state.json`。兩者都出現過，可能是不同程式路徑各自寫入。

**形態 B — turn 已完成並計費，但輸出永久遺失**

一個 job 的記錄為：

```
status: "failed"，phase: "failed"
failure.summary: "EPERM: ... rename ... -> ...\state.json"
```

而它的 `.log` 只有：

```
[10:58:32] Starting Gemini Task.
[10:58:32] Queued for background execution.
[10:58:33] Detecting engine...
[10:58:34] Starting agy turn...
[10:59:26] Turn completed.
```

**`Turn completed.` 有寫入，但 `Final output` 沒有。** 模型跑了 52 秒、已經計費，輸出卻在寫入前就被 EPERM 中斷，`.log` 裡沒有、`<jobId>.json` 裡也沒有。這一份輸出**無法用任何方式取回**，只能整批重跑。

這比 #4 嚴重一個等級：#4 的輸出還躺在 `.log` 裡可以手動挖出來，形態 B 是真正的資料遺失。

**追加的期望行為**

4. 寫入 `Final output` 應該先於任何共用 state 的更新，且兩者要分開失敗。模型輸出是這個系統裡唯一無法重建的東西（重跑會得到不同結果，而且要再付一次費用），它的持久化不該取決於一個會因併發而失敗的 rename。
5. 形態 A 的失敗訊息應直接說明是併發衝突並建議降低併發數，而不是拋出原始的 `EPERM` 字串。

**本輪統計**：27 個 job 中 2 個受 EPERM 影響（7.4%），其中 1 個為形態 B 的永久遺失。9 秒間隔比第一輪的 <1 秒改善很多，但**沒有消除問題**——只要同時有 20 個以上的 job 在跑，仍會撞上。實務上的規避方式是把批次拆小並分波啟動，等前一波收完再送下一波。
