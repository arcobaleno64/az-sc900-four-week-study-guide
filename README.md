...
- 四週二十八天是否完整且連續。
- 每科是否至少有一題，且題號從一開始連續、不重複。
- 每題是否具有有效的題型、答案、解析、陷阱、技能領域與官方來源 ID。
- 單選、複選與是非題的判分規則是否通過自動測試。
...
3. 更新 `data/exam-meta.json` 的 `lastVerified` 與生效日期；題數由 `data/questions.json` 自動計算。
